...
@property (nonatomic, strong) CALayer *seratorLayer;
...
{
    XLTabItem *item = [super buttonWithType:buttonType];
    [item setup];
    return item;
}
- (void)setup
{
    ...
    _badgeStyle = XLTabItemBadgeStyleNumber;
    self.badge = 0;
}
/**
 *  @author YASIN, 16-08-22 15:08:18
 *
 *  覆盖父类的setHighlight:方法，按下XLTapItem时，不高亮Item
 */
...
- (void)setBadgeStyle:(XLTabItemBadgeStyle)badgeStyle
{
...
- (void)updateBadge
{
    if (self.badgeStyle == XLTabItemBadgeStyleNumber) {
        if (self.badge == 0) {
            ...
            // 宽度width和height的较大值，使badge为个位数时，badgeButton为圆形
    ...
    } else if (self.badgeStyle == XLTabItemBadgeStyleDot) {
