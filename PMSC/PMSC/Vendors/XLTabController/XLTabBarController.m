...
- (void)setViewControllers:(NSArray *)viewControllers {
    ...
    for (UIViewController *controller in _viewControllers) {
        XLTabItem *item = [XLTabItem buttonWithType:UIButtonTypeCustom];
        item.image = controller.XL_tabItemImage;
        item.selectedImage = controller.XL_tabItemSelectedImage;
        item.title = controller.XL_tabItemTitle;
...
- (void)XL_tabBar:(XLTabBar *)tabBar didSelectedItemAtIndex:(NSInteger)index {
...
@implementation UIViewController (XLTabBarController)

- (NSString *)XL_tabItemTitle {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setXL_tabItemTitle:(NSString *)XL_tabItemTitle {
    objc_setAssociatedObject(self, @selector(XL_tabItemTitle), XL_tabItemTitle, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (UIImage *)XL_tabItemImage {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setXL_tabItemImage:(UIImage *)XL_tabItemImage {
    objc_setAssociatedObject(self, @selector(XL_tabItemImage), XL_tabItemImage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIImage *)XL_tabItemSelectedImage {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setXL_tabItemSelectedImage:(UIImage *)XL_tabItemSelectedImage {
    objc_setAssociatedObject(self, @selector(XL_tabItemSelectedImage), XL_tabItemSelectedImage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (XLTabItem *)XL_tabItem {
    XLTabBar *tabBar = self.XL_tabBarController.tabBar;
    NSInteger index = [self.XL_tabBarController.viewControllers indexOfObject:self];
    return tabBar.items[index];
}

- (XLTabBarController *)XL_tabBarController {
    return (XLTabBarController *)self.parentViewController;
}
