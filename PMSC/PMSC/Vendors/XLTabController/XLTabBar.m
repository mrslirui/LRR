...
@property (nonatomic, strong) XLTabItem *specialItem;
@property (nonatomic, copy) void (^specialItemHandler)(XLTabItem *item);

// 选中背景
@property (nonatomic, strong) UIImageView *itemSelectedBgImageView;

// 选中背景相对于XLTabItem的insets
...
- (void)setItems:(NSArray *)items {
    ...
    // 初始化每一个item
    for (XLTabItem *item in self.items) {
...
- (void)setTitles:(NSArray *)titles {
    ...
    for (NSString *title in titles) {
        XLTabItem *item = [[XLTabItem alloc] init];
        item.title = title;
...
- (void)updateItemsFrame {
    ...
    if (self.scrollView) {
        ...
        for (NSInteger index = 0; index < self.items.count; index++) {
            XLTabItem *item = self.items[index];
            CGFloat width = 0;
            // item的宽度为一个固定值
            if (self.itemWidth > 0) {
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (_selectedItemIndex >= 0) {
        XLTabItem *oldSelectedItem = self.items[_selectedItemIndex];
    ...
    XLTabItem *newSelectedItem = self.items[selectedItemIndex];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(xl_tabBar:didSelectedItemAtIndex:)]) {
        [self.delegate xl_tabBar:self didSelectedItemAtIndex:selectedItemIndex];
    }
...
- (void)tabItemClicked:(XLTabItem *)item {
    if (self.selectedItemIndex == item.index) {
        return;
    }
    BOOL will = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(xl_tabBar:willSelectItemAtIndex:)]) {
        will = [self.delegate xl_tabBar:self willSelectItemAtIndex:item.index];
    }
    if (will) {
...
- (void)specialItemClicked:(XLTabItem *)item {
...
- (void)setSpecialItem:(XLTabItem *)item afterItemWithIndex:(NSInteger)index tapHandler:(void (^)(XLTabItem *item))handler {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    XLTabItem *leftItem = self.items[leftIndex];
    XLTabItem *rightItem = nil;
...
- (void)updateSeperators {
    if (self.itemSeparatorColor) {
        ...
        [self.items enumerateObjectsUsingBlock:^(XLTabItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx > 0) {
